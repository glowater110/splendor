...
class SplendorEnv4PP2G2(gym.Env):
    """
    Splendor Environment 4P P2G2: Policy 2 (Point-Focus), Generation 2.
    The Agent trains against a P2G1 model (loaded from file).
    Rewards:
    - Win: +100
    - Lose: -100
    ...
    def __init__(self, num_players=4, opponent_model_path="ai_4p_p2g1.zip"):
        super(SplendorEnv4PP2G2, self).__init__()
    ...
    def step(self, action_idx):
        ...
        # 2. Opponents' Turns (Controlled by P2G1 Model or Random)
        if self.game.curr_player_idx != self.agent_idx:
            while self.game.curr_player_idx != self.agent_idx and not self.game.game_over:
                ...
                # Auto-discard logic handled by model (via mask) or game.step for random?
                # game.step DOES NOT auto-discard. If random, we need to handle it.
                # If model, it should have predicted discard.
                # BUT if model takes tokens -> >10 -> turn doesn't advance.
                # Next loop iteration: same player. Model sees "Must Discard" mask. Predicts discard.
                # So we just loop.
